...
export function CollectionManager({
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !body?.collection) {
        setError(body?.error?.message ?? 'We could not create that collection.');
        return;
      }
  ...
  async function move(collectionId: string, direction: -1 | 1) {
    ...
    try {
      ...
      if (!response.ok || !body?.collections) {
        setError(body?.error?.message ?? 'We could not save that collection order.');
        return;
      }
  ...
  async function remove(collection: CollectionSummary) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(body?.error?.message ?? 'We could not remove that collection.');
        return;
      }
  ...
  return (
    <main className="settings-page collection-page">
      ...
      {collections.length ? (
        ...
        <section className="empty-library collection-empty">
          <h2>Make the first shelf.</h2>
          <p>
            Collections are a calm way to gather seasonal menus, favorites, and family traditions.
          </p>
        </section>
      )}
    </main>

...
import { nutritionMealAllocationInputSchema } from '@/lib/domain/nutrition-intake';
import {
...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    ...
    return NextResponse.json({
      allocations: listNutritionMealAllocationVersions(profileId, auth.principal.id),
    });
  } catch (error) {
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    const profileId = parseNutritionProfileId((await context.params).profileId);
    const input = nutritionMealAllocationInputSchema.parse(await readJson(request));
    return NextResponse.json(
      {
        allocation: appendNutritionMealAllocationVersion(profileId, auth.actor, input),
      },
      { status: 201 },
    );
  } catch (error) {

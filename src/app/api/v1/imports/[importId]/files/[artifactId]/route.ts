...
export async function GET(
  _request: Request,
  context: { params: Promise<{ importId: string; artifactId: string }> },
) {
  try {
    const { importId, artifactId } = await context.params;
    const artifact = await getImportArtifact(importId, artifactId);

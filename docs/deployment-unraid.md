...
For upgrades, pull the new image and recreate the container while keeping the same `/data` mapping. The PWA checks `/sw.js` without an intermediary cache, activates the current worker immediately, deletes older `our-recipes-read-*` caches, and reloads open clients onto the current bundle. A one-time DevTools service-worker purge should not be required for releases containing this update lifecycle.

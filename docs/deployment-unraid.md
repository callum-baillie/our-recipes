...
1. Create `/mnt/user/appdata/our-recipes` and make it writable by UID/GID `1001` (or adjust host ownership before first start).
...
3. Map container `/data` to `/mnt/user/appdata/our-recipes` and map container port `3000` to the chosen LAN port.
4. Set a unique `COOKIE_SECRET`, exact `APP_ORIGIN` such as `http://recipes.lan:3000`, timezone, and optional trusted proxy origins. Set `OPENAI_API_KEY` only when you intentionally permit explicit OpenAI actions; do not mount or copy the development-only `.api_keys` file.
